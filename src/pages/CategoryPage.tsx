...
// This could be moved to a separate data file in a real application
const products = {
  skincare: [
    {
      id: '1',
...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    if (category && products[category as keyof typeof products]) {
      setCategoryProducts(products[category as keyof typeof products]);
    } else {
      toast({
        title: "Category not found",
        description: `Could not find products for ${category}`,
        variant: "destructive"
      });
      setCategoryProducts([]);
    }
      ...
      <main className="flex-grow pt-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center mb-12 animate-reveal-delay-1 opacity-0">
            <h1 className="text-4xl md:text-5xl font-serif font-medium text-cane-950 capitalize">
              {category} Collection
            </h1>
            <p className="mt-4 max-w-2xl mx-auto text-cane-700">
              Discover our collection of premium {category === 'skincare' ? 'skincare products' : 'health supplements'} designed to enhance your natural beauty and wellbeing.
            </p>
          </div>
                ...
                <div className="space-y-2">
                  {category === 'skincare' ? (
                    ...
                    <>
                      <label className="flex items-center">
                        <input type="checkbox" className="rounded text-cane-950 focus:ring-cane-500" />
                        <span className="ml-2 text-sm">Immunity</span>
                      </label>
                      <label className="flex items-center">
                        <input type="checkbox" className="rounded text-cane-950 focus:ring-cane-500" />
                        <span className="ml-2 text-sm">Energy</span>
                      </label>
                      <label className="flex items-center">
                        <input type="checkbox" className="rounded text-cane-950 focus:ring-cane-500" />
                        <span className="ml-2 text-sm">Stress Relief</span>
                      </label>
                    </>
                  )}
                </div>
          ...
          {categoryProducts.length === 0 && (
            <div className="text-center py-12">
              <p className="text-cane-600">No products found in this category.</p>
            </div>
          )}
        </div>
      </main>
